...
function CheckoutContent() {
    ...
    return (
        <div className="min-h-screen bg-[#050505] text-[#f5f5f5] flex flex-col items-center overflow-hidden">
            ...
            {/* Navigation Strip */}
            <motion.div initial="hidden" animate="visible" variants={fadeUp} className="w-full relative z-20 mt-6 lg:mt-8">
            ...
            <div className="flex-1 w-full max-w-[1200px] mx-auto px-6 py-12 md:py-20 relative z-10 flex flex-col items-center">
                <motion.div
                    ...
                    animate="visible"
                    variants={fadeUp}
                    className="w-full max-w-4xl grid md:grid-cols-2 gap-8 items-start mb-auto"
                >
                    {/* Order Summary */}
                    <div className="space-y-6">
                        <div>
                            <h1 className="text-3xl lg:text-4xl font-extrabold tracking-tight mb-3">Finalizacja Zamówienia</h1>
                            <p className="text-slate-400 text-lg">Wybrałeś pakiet <span className="text-accent-red font-bold">{plan.name}</span>.<br /> Skonfigurujmy Twoje środowisko.</p>
                        </div>

                        <Card className="p-6 border-white/5 bg-white/5 backdrop-blur-xl rounded-[1.5rem]">
                            <div className="flex justify-between items-center mb-6">
                                <span className="text-xs font-bold uppercase tracking-widest text-slate-500">Plan</span>
                                <span className="px-3 py-1.5 rounded-md bg-accent-red/10 text-accent-red border border-accent-red/20 text-xs font-bold tracking-wider">{plan.name}</span>
                            </div>
                            <ul className="space-y-4 mb-8">
                                {plan.features.map((f, i) => (
                                    <li key={i} className="flex items-center gap-3 text-sm text-slate-300">
                                        <Check size={16} className="text-emerald-500 shrink-0" /> {f.text}
                                    </li>
                                ))}
                            </ul>
                            <div className="pt-6 border-t border-white/5 flex justify-between items-end">
                                <span className="text-slate-500 text-sm font-medium">Do zapłaty:</span>
                                <div className="text-3xl font-black tracking-tight">{displayPrice} zł <span className="text-xs text-slate-500">/{isYearly ? 'rok' : 'msc'}</span></div>
                            </div>
                        </Card>

                        <div className="flex items-center gap-4 p-4 rounded-xl bg-blue-500/5 border border-blue-500/10 text-blue-400/80 text-xs">
                    ...
                    {/* Stripe Elements UI */}
                    <Card className="p-8 border-white/10 bg-[#0a0a0a]/90 backdrop-blur-2xl shadow-2xl relative min-h-[400px] flex flex-col justify-center rounded-[1.5rem] overflow-hidden">
                        {/* Form Glow */}
                        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[300px] h-[300px] bg-red-500/5 blur-[80px] rounded-full pointer-events-none"></div>

                        {error ? (
                            <div className="text-center p-6 space-y-4 relative z-10">
                                <div className="w-12 h-12 rounded-2xl bg-red-500/10 border border-red-500/20 flex items-center justify-center mx-auto text-red-500 shadow-lg shadow-red-500/20">
                                    <Lock size={20} />
                                </div>
                                <h3 className="font-bold text-lg text-white">Błąd Płatności</h3>
                                <p className="text-sm text-red-400/80">{error}</p>
                                <Button variant="secondary" onClick={() => window.location.reload()} className="w-full mt-2">Spróbuj Ponownie</Button>
                            </div>
                        ) : (isInitLoading || !clientSecret || !publishableKey) ? (
                            <div className="flex flex-col items-center justify-center gap-6 relative z-10 py-12">
                            ...
                            <div className="relative z-10">
                                <div className="mb-8">
                                    ...
                                    <p className="text-slate-500 text-sm">Wprowadź dane karty i aktywuj Twoje konto.</p>
                                </div>
                                <div className="p-1">
                ...
                <motion.div initial="hidden" animate="visible" variants={fadeUp} className="flex justify-center gap-2 mt-16 relative z-10">
