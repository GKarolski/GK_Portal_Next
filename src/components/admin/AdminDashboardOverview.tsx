...
export const AdminDashboardOverview: React.FC<AdminDashboardOverviewProps> = ({ tickets, clients, onNavigateToClient, onOpenTicket }) => {
        ...
        return (period: number) => {
            ...
            doneWithPrice.forEach(ticket => {
                const ticketDate = (ticket as any).billingMonth
                    ...
                    || new Date(ticket.createdAt).toISOString().substring(0, 7);
    ...
    const currentMonthRevenue = doneWithPrice
        .filter(t => {
            const ticketDate = (t as any).billingMonth || (t as any).billing_month || new Date(t.createdAt).toISOString().substring(0, 7);
